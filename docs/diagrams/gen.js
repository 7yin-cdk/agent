...
function healthcheckFlow() {
  ...
  const P = PALETTE;
  let b = '';
  b += title(W, '数据库健康巡检流程（定时 + 代码预取 + LLM + 规则兜底）');
  b += footer(W, H, 'HealthCheckRunner · @Scheduled 触发 → 代码预取指标+规则判定 → ReAct 定位根因并告警 → 逐指标兜底补发 → HealthCheckRecord 落库 + Trace 持久化');

  const sx = 180, sw = 560, cx = sx + sw / 2;
  const rx = 780, rw = 260;

  b += step(sx, 70, sw, 52, 1, '触发巡检', ['@Scheduled(cron 可配置) 或 POST /healthcheck/run 手动'], P.app);
  ...
  b += varrow(cx, 202, 230, '', '#fb8c00');
  /* 指标是确定性数据，改由代码预取并顺带完成规则判定，模型不必再调工具取数 */
  b += step(sx, 230, sw, 56, 3, '代码预取指标 + 规则判定', ['DatabaseMetricsCollector.collect + AnomalyEvaluator.evaluate'], P.app);
  b += sideNote(780, 254, 260, '活跃会话 / 缓冲池命中率 / 锁等待 / 死元组 / 复制延迟 …', '#455a64');
  b += varrow(cx, 286, 314, '', '#fb8c00');
  b += step(sx, 314, sw, 56, 4, '构建巡检 Prompt buildPrompt', ['注入每实例指标 JSON + 判定结论 / 可用工具 / 阈值 / 执行步骤 / 输出格式'], P.app);
  b += varrow(cx, 370, 398, '', '#ab47bc');
  b += step(sx, 398, sw, 52, 5, 'LLM ReAct 分析 runLlmReAct', ['ToolCallingService.chatWithTasks（带 Trace 采集）'], P.domain);
  b += box(rx, 398, rw, 122, 'ReAct 循环内', ['· 下钻工具定位根因（指标已预取）', '· sendAlertEmail 告警并声明覆盖指标', '· 返回巡检总结 llmSummary'], P.domain, 13, 11);
  b += varrow(cx, 450, 478, '', '#fb8c00');
  b += step(sx, 478, sw, 52, 6, '对每个实例执行以下循环', ['persistAndFallback(runId, snapshot, llmSummary)'], P.app);
  b += `<rect x="150" y="558" width="620" height="246" rx="10" fill="none" stroke="#78909c" stroke-width="1.3" stroke-dasharray="6,4"/>`;
  b += `<text x="160" y="578" font-family="${FONT}" font-size="12.5" font-weight="bold" fill="#455a64">循环：对每个配置的 Target 实例（复用第 3 步快照）</text>`;
  b += step(sx, 590, sw, 50, 7, '兜底判定 AlertCoverageResolver.decide', ['规则异常项 − 模型已声明覆盖项 = 待补发项'], P.app);
  b += sideNote(780, 614, 260, '无覆盖声明 → 视为实例级已覆盖，不补发', '#c62828');
  b += varrow(cx, 640, 662, '', '#fb8c00');
  b += step(sx, 662, sw, 50, 8, '邮件补发兜底 sendFallback', ['差集非空 / 实例连接失败 → 发送补充告警邮件'], P.app);
  b += sideNote(780, 686, 260, '发送成功 → markSent(runId, 补发指标)', '#00695c');
  b += varrow(cx, 712, 734, '', '#fb8c00');
  b += step(sx, 734, sw, 50, 9, '落库 HealthCheckRecord', ['status: NORMAL / ANOMALY / ERROR · abnormalMetrics JSON'], P.app);
  b += varrow(cx, 784, 812, '', '#fb8c00');
