...
version: "18"
source: "official"
source_url: "https://www.postgresql.org/docs/18/progress-reporting.html"
document: "progress-reporting"
title: "PostgreSQL: Documentation: 18: 27.4. Progress Reporting"
chapter: "PostgreSQL: Documentation: 18: 27.4. Progress Reporting"
---
...
| `phase` `text`  Current processing phase. See Table 27.39. |
...
| `delay_time` `double precision`  Total time spent sleeping due to cost-based delay (see Section 19.10.2), in milliseconds (if track\_cost\_delay\_timing is enabled, otherwise zero). |
...
| `phase` `text`  Current processing phase. See Table 27.41. |
...
| `max_dead_tuple_bytes` `bigint`  Amount of dead tuple data that we can store before needing to perform an index vacuum cycle, based on maintenance\_work\_mem. |
...
| `vacuuming indexes` | `VACUUM` is currently vacuuming the indexes. If a table has any indexes, this will happen at least once per vacuum, after the heap has been completely scanned. It may happen multiple times per vacuum if maintenance\_work\_mem (or, in the case of autovacuum, autovacuum\_work\_mem if set) is insufficient to store the number of dead tuples found. |
