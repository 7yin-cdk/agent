...
version: "18"
source: "official"
source_url: "https://www.postgresql.org/docs/18/glossary.html"
document: "glossary"
title: "PostgreSQL: Documentation: 18: Appendix M. Glossary"
chapter: "PostgreSQL: Documentation: 18: Appendix M. Glossary"
---
...
Base Backup
:   A binary copy of all database cluster files. It is generated by the tool pg\_basebackup. In combination with WAL files it can be used as the starting point for recovery, log shipping, or streaming replication.
...
Incremental backup
:   A special base backup that for some files may contain only those pages that were modified since a previous backup, as opposed to the full contents of every file. Like base backups, it is generated by the tool pg\_basebackup.

    To restore incremental backups the tool pg\_combinebackup is used, which combines incremental backups with a base backup. Afterwards, recovery can use WAL to bring the database cluster to a consistent state.
...
:   Byte offset into the WAL, increasing monotonically with each new WAL record.
    ...
    For more information, see pg\_lsn and Section 28.6.
...
WAL sender (process)
:   A special backend process that streams WAL over a network. The receiving end can be a WAL receiver in a replica, pg\_receivewal, or any other client program that speaks the replication protocol.
